...
$logoft = $d->rawQueryOne("select id, photo from #_photo where type = ? and act = ? and hienthi > 0 limit 0,1",array('logo-ft','photo_static'));
$logock = $d->rawQueryOne("select id, photo from #_photo where type = ? and act = ? and hienthi > 0 limit 0,1",array('logo-ck','photo_static'));
...
$sloganndth = $d->rawQueryOne("select ten$lang as ten from #_static where type = ? limit 0,1",array('slogan-ndth'));
$sloganndth2 = $d->rawQueryOne("select ten$lang as ten from #_static where type = ? limit 0,1",array('slogan-ndth2'));
...
$giomocua = $d->rawQueryOne("select ten$lang as ten, noidung$lang as noidung from #_static where type = ? limit 0,1",array('giomocua'));
$nhanuudaigiamgia = $d->rawQueryOne("select ten$lang as ten, noidung$lang as noidung from #_static where type = ? limit 0,1",array('nhan-uu-dai-giam-gia'));
...
$huongdanthongtin = $d->rawQuery("select ten$lang as ten, tenkhongdauvi, tenkhongdauen, mota$lang as mota, ngaytao, id, photo from #_news where type = ? and hienthi > 0 order by stt,id desc",array('huong-dan-thong-tin'));
$pttt = $d->rawQuery("select ten$lang as ten, photo, link from #_photo where type = ? and hienthi > 0 order by stt,id desc",array('pttt'));
