    ...
    <div class="container-fluid">
        <div class="row">
            <ol class="breadcrumb float-sm-left">
                ...
                <li class="breadcrumb-item active">Cập nhật <?=$config['search']['man_search'][$type]['title_main_search']?></li>
            </ol>
        </div>
    </div>
</section>
    ...
    <form method="post" action="<?=$linkSave?>" enctype="multipart/form-data">
        ...
        <div class="card card-primary card-outline text-sm">
            <div class="card-header">
                <h3 class="card-title">Chi tiết <?=$config['search']['man_search'][$type]['title_main_search']?></h3>
            </div>
            <div class="card-body">
                            ...
                            <div class="card-body">
                                ...
                                <?php if(isset($config['search']['man_search'][$type]['min_search']) && $config['search']['man_search'][$type]['min_search'] == true) { ?>
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control format-price" name="data[min]" id="min" placeholder="Min" value="<?=@$item['min']?>">
                                    </div>
                                <?php } ?>
                                <?php if(isset($config['search']['man_search'][$type]['max_search']) && $config['search']['man_search'][$type]['max_search'] == true) { ?>
                                    <div class="form-group">
                                        <label for="max">Max:</label>
                                        <input type="text" class="form-control format-price" name="data[max]" id="max" placeholder="Max" value="<?=@$item['max']?>">
                                    </div>
                                <?php } ?>
                                ...
                                <?php if(isset($config['search']['man_search'][$type]['tygia_search']) && $config['search']['man_search'][$type]['tygia_search'] == true) { ?>
                                    <div class="form-group">
                                        ...
                                        <input type="text" class="form-control format-price" name="data[tygia]" id="tygia" placeholder="Tỷ giá" value="<?=@$item['tygia']?>">
                                    </div>
                                <?php } ?>
                            </div>
                        </div>
