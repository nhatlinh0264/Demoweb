...
function save_item()
{
	...
	/* Kiểm tra username */
	$username = isset($data['username']) ? $data['username'] : '';
	$check_username = $d->rawQueryOne("select id form #_member where username = ? and id <> ? limit 0,1",array($username, $id));
	if(!empty($check_username)) $func->transfer("Tên đăng nhập này đã tồn tại. Xin chọn tên khác", "index.php?com=user&act=man&p=".$curPage, flase);
